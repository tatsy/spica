...
template <class T>
spica::Point3_<T> operator*(T s, spica::Point3_<T>& p) {
...
template <class T>
spica::Vector3_<T> operator*(const spica::Normal3_<T>& n, T s) {
    return { s * n.x(), s * n.y(), s * n.z() };
}

template <class T>
spica::Vector3_<T> operator*(T s, const spica::Normal3_<T>& n) {
    return { s * n.x(), s * n.y(), s * n.z() };
}
